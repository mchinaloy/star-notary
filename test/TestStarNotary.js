...
it('can add the star name and star symbol properly', async() => {
    ...
    assert.equal("Dogestar", actualName);
    assert.equal("DSR", actualSymbol);
});
...
it('lets 2 users exchange stars', async() => {
    ...
    assert.equal(user2, await instance.ownerOf(tokenId1));
    assert.equal(user1, await instance.ownerOf(tokenId2));
});
...
it('lets a user transfer a star', async() => {
    ...
    assert.equal(name, await instance.lookUptokenIdToStarInfo(tokenId, {from: user2}));
});
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let instance = await StarNotary.deployed();
    let name = 'My Cool Star';
    let tokenId = 11;
    await instance.createStar(name, tokenId);
    assert.equal(name, await instance.lookUptokenIdToStarInfo(tokenId)); 
});